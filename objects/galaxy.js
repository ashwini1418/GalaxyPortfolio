...
import { cos, float, mix, range, sin, time, uv, vec3, vec4, color } from 'three/tsl';
...
export function createGalaxy() {
    ...
    const branchAngle =
        range(0,branches)
        .floor()
        .mul(float(Math.PI * 2 / branches));

    const spin = -9;

    const angle =
        branchAngle
        .add(radiusRatio.mul(spin))
        .add(time.mul(radiusRatio.oneMinus().mul(0.1)));
